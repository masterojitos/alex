        ...
        return this.each(function() {
            ...
            var defaults = {
                ...
                upload_success_handler: function(file, response) {
					...
					$("span[id$=_completedMessage]", container).html("Uploaded <b><a href='" + options.path_url + response + "' rel='shadowbox;" + (file.name.indexOf(".flv") >= 0 ? "width=480;height=240;" : "") + "'>" + file.name + "</a></b> (" + Math.round(file.size / 1024) + " KB) ");
					Shadowbox.setup();
                },
            ...
            if (options.existingFilename || "" != ""){
				$("span[id$=_completedMessage]", container).html("Uploaded <b><a href='" + options.path_url + options.existingGuid + "' rel='shadowbox;" + (options.existingFilename.indexOf(".flv") >= 0 ? "width=480;height=240;" : "") + "'>" + options.existingFilename + "</a></b> (" + (options.existingFileSize ? Math.round(options.existingFileSize / 1024) : "?") + " KB) ");
